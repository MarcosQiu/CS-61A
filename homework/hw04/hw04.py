...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert is_mobile(m), "planet has no concept of balance"
    left_end = end(left(m))
    right_end = end(right(m))
    return (
            length(left(m)) * total_weight(left_end) == length(right(m)) * total_weight(right_end)
    ) and (is_planet(left_end) or balanced(left_end)) and (is_planet(right_end) or balanced(right_end))
...
def totals_tree(m):
    ...
    left_child = end(left(m))
    right_child = end(right(m))

    children = [totals_tree(left_child), totals_tree(right_child)]
...
def replace_thor_at_leaf(t, thors_replacement):
    ...
    if is_leaf(t):
        ...
        return tree(
            label(t),
            [replace_thor_at_leaf(b, thors_replacement) for b in branches(t)]
        )
