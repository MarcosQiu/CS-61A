...
def missing_digits(n):
    ...
    True
    """
    def helper(num, last_digit):
        if num < 10:
            return max(last_digit - num - 1, 0)
        return helper(num // 10, num % 10) + max((last_digit - num % 10 - 1), 0)
...
def count_coins(change):
    ...
    True
    """
    def helper(change, coin_to_use):
        if get_next_coin(coin_to_use) is None:
            return 0 if change % coin_to_use > 0 else 1
        if change < 0:
            return 0
        if change == 0:
            return 1
        return helper(change - coin_to_use, coin_to_use) + helper(change, get_next_coin(coin_to_use))
