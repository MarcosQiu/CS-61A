...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    squared_sum = score ** 2
    smallest_digit = squared_sum % 10
    while squared_sum > 0:
        smallest_digit = min(smallest_digit, squared_sum % 10)
        squared_sum = squared_sum // 10
...
def more_boar(player_score, opponent_score):
    ...
    return (
        (player_score % 10 < opponent_score % 10) and
        (player_score // 10 < opponent_score // 10)
    )
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        playerPoints, opponentPoints = (score1, score0) if who else (score0, score1)
        strategy = strategy1 if who else strategy0
        while True:
            numOfDices = strategy(playerPoints, opponentPoints)
            playerPoints += take_turn(numOfDices, opponentPoints, dice, goal)
            score1, score0 = (playerPoints, opponentPoints) if who else (opponentPoints, playerPoints)
            say = say(score0, score1)
            if (
                playerPoints >= goal or
                not more_boar(playerPoints, opponentPoints)
            ):
                break
        who = next_player(who)
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def announce(score0, score1):
        player_score = score1 if who else score0
        score_this_turn = player_score - last_score
        if score_this_turn > running_high:
            print('Player %d has reached a new maximum point gain. %d point(s)!' % (who, score_this_turn))
            return announce_highest(who, player_score, score_this_turn)
        return announce_highest(who, player_score, running_high)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for rolls in range(1, 11):
        ave_score = trial_func(rolls, dice)
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if more_boar(
            piggy_points(opponent_score) + score,
            opponent_score
        ) else piggypoints_strategy(
            score,
            opponent_score,
            cutoff,
            num_rolls
